...
var SimpleDB = exports.Simpledb = function(options) {
    ...
    if(!this.domainPrefix) {
        throw new Error('No domain prefix! This is a required field.')
    }
    ...
    this.sdb.createDomain(this.domainPrefix + this.resource, function(err, res, meta) {
        if(err)
        {
            log.error(err)
            throw err;
...
SimpleDB.prototype.get = function(item, callback) {
    ...
    this.sdb.getItem(domainName, id, function(err, res, meta) {
        if(err) {
            ...
            if(!res) {
                callback({status:404, message: "resource not found"}, {is:'broken'});
                return;
...
SimpleDB.prototype.save = function (id, data, callback) {
    ...
    log.debug("Saving to " + domainName + "..." + util.inspect(data));
